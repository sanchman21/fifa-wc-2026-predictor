    ...
    for date in dates:
        ...
        for ev in payload.get("events", []):
            ...
            if status != "STATUS_FULL_TIME":
                continue  # full-time only: never write in-play or scheduled scores
...
def merge_live_into_results(results_path: str | None = None,
    ...
    """Overlay ESPN full-time scores onto NA WC-2026 fixture rows in results.csv.
    ...
    orients the scores to the matched CSV row's home/away. Writes the CSV back in the same
    format (columns, header, NA/TRUE/FALSE representation) and verifies by re-reading.

    Returns: {filled, matched_already_present, unmapped, details}.
    """
    ...
    for m in live:
        ...
        if not cands:
            unmapped.append({**m, "home_dn": home_dn, "away_dn": away_dn,
                             "reason": "no WC fixture with this team pair"})
            continue
    ...
    if filled:
        ...
        for d in details:
            sel = check[(check["date"].astype(str).str[:10] == d["date"])
...
if __name__ == "__main__":
    ...
    print(f"\nFilled {res['filled']} new full-time score(s); "
